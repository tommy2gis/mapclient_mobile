...
var ComputeDist = L.Control.extend({
    options: {
        distoolbarIcon: { className: 'leaflet-ComputeDist' }
    },
    _rad: Math.PI / 180,
    ...
    onAdd: function(map, options) {
        ...
        var iconOptions = this.options.distoolbarIcon;

        this._icon = L.DomUtil.create('div','measure_btn');
        this._link = L.DomUtil.create('a', '');

        this._icon.innerHTML = iconOptions.html;
        ...
        L.DomEvent.on(this._icon, 'click', this._onMouseClick, this);
    ...
    _onMouseClick: function () {
        this._startCompute && this._finishCompute();
...
var ComputeArea = L.Control.extend({
    options: {
        areatoolbarIcon: { className: 'leaflet-ComputeArea' }
    },
    _rad: Math.PI / 180,
    _deg: 180 / Math.PI,
    onAdd: function(map, options) {
        ...
        //  this._map._areaControl = this;
          L.setOptions(this, options);
          var iconOptions = this.options.areatoolbarIcon;
    ...
    _bearing: function (a, b) {
        var degToRad = this._rad,
            ...
            phi2 = b.lat * degToRad,
            lam2 = b.lng * degToRad;
        var angle = -Math.atan2(Math.sin(lam1 - lam2) * Math.cos(phi2), Math.cos(phi1) * Math.sin(phi2) - Math.sin(phi1) * Math.cos(phi2) * Math.cos(lam1 - lam2));
...
module.exports ={ComputeArea,ComputeDist}